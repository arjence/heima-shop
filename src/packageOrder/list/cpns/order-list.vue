...
import type { ListItem, OrderListResult } from '@/types/order'
...
const orderInfo = ref<OrderListResult>()
const finish = ref(false)
let page = 0
const getOderListData = async () => {
  if (finish.value) return
  const { result } = await getMemberOrderApi<OrderListResult>({
    page,
    pageSize: 10,
...
const onDeleteOrder = async (id: string) => {
  uni.showModal({
    ...
    success: async (success) => {
      if (success) {
        await deleteMemberOrderByIdApi({ ids: [id] })
        const index = orderInfo.value?.items.findIndex((item) => item.id === id)
        orderInfo.value?.items.splice(index!, 1)
      }
    },
  })
}

const onConfirmOrder = (item: ListItem) => {
  uni.showModal({
    content: '为保障您的权益，请收到货并确认无误后，再确认收货',
    success: async (confirm) => {
      if (confirm) {
        await putMemberOrderReceiptByIdApi(item.id)
...
const refresherTrigger = ref(false)
...
<template>
  ...
  <scroll-view
    ...
    @refresherrefresh="onRefresherefresh"
  >
    <view class="card" v-for="item in orderInfo?.items" :key="item.id">
      <!-- 订单信息 -->
      <view class="status">
      ...
      <navigator
        ...
        :key="sku.spuId"
        class="goods"
        :url="`packageOrder/detail/detail?id=${sku.id}`"
        hover-class="none"
      >
        <view class="cover">
      ...
      <view class="action">
        ...
        <template v-else>
          <navigator
            ...
            :url="`/packageOrder/create/create?orderId=${item.id}`"
            hover-class="none"
          >
            再次购买
          </navigator>
